  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full">
        <div className="flex z-10 w-full flex-col items-start pt-5 md:pt-0 gap-3 md:flex-row">
                  ...
                  <div className="relative w-full">
                    <FormControl>
                      <Input
                        ...
                        type="text"
                        id="emailInput"
                        placeholder={scopedT("enterYourAddress")}
                        className="block h-fit w-full rounded-lg py-5 pr-5 pl-11 text-primary"
                      />
                    </FormControl>
                    <MapPin className="absolute top-1/2 left-4 size-5 -translate-y-1/2 stroke-gray-400" />
                  </div>
          ...
          <div className="w-full shrink-0 md:w-fit">
            <Link href={routes.category.categories}>
              <Button
                //  type="submit"
                className="h-fit w-full cursor-pointer rounded-lg px-8 py-5 font-semibold text-nowrap md:w-fit text-white "
              >
                {scopedT("startOrdering")}
                {/*{scopedT("currentAddress")} */}
              </Button>
            </Link>
          </div>
        </div>
      ...
      <motion.div ref={scope}>
        {wordsArray.map((word, idx) => {
          return (
            <motion.span
              key={word + idx}
              className="dark:text-white text-black opacity-0 font"
              style={{
                filter: "blur(10px)",
              }}
            >
  ...
  return (
    <section className="flex font-dm_sans bg-background py-12 md:py-20 items-center justify-center ">
      <div className="container max-w-7xl">
        <div className="grid grid-cols-1 items-center pt-10 lg:pt-0 justify-center gap-20 lg:grid-cols-2">
          <div>
            <motion.div
              initial={{ x: -1000, opacity: 0 }}
              animate={{ x: 0, opacity: 1 }}
              transition={{ type: "spring", duration: 2.5 }}
              className="flex px-4 flex-col gap-8"
            >
              <h1 className="font-serif inline text-4xl text-foreground lg:text-5xl xl:text-5xl">
                {renderWords()}
              </h1>
              <p className="text-lg text-balance inline text-muted-foreground">
                {scopedT("orderWaitReceive")}
              </p>
              <HeroFrom />
            </motion.div>
          </div>
          <div>
            <motion.div
              ...
              animate={{ x: 0, opacity: 1 }}
              transition={{ type: "spring", duration: 2.5 }}
              className="relative mx-auto aspect-[0.990343348_/_1] w-full max-w-[42.5rem]"
            >
              <div
                style={{ transform: "translateY(calc(var(--scroll-y) * 0.2))" }}
                className="absolute top-[5.04%] left-[4.33%] z-20 w-[42.25%]"
              >
                <AspectRatio
                  ratio={0.857142857 / 1}
